...
struct PLAYER_NAME : public Player {
    ...
    /* VECTORS */
        ...
        /* CONST */
            ...
            /* WITCH */
                ...
                /* KILL KNIGHT */
                    const double WK_KNIGHT = 30.0;
                    const double WK_KNIGHT_C = 1.5;
                    const int WK_KNIGHT_D = 13;

                /* WITCH OVERLAP */
                    const double WO_WITCH = -50.0;
                    const double WO_WITCH_C = 4.0;
            ...
            /* FARMER */
                /* RUN FROM WITCH */
                    ...
                    const double FR_FWITCH = -8000;
                    const double FR_FWITCH_C = 4;
                    const int FR_FWITCH_D = 5;

                /* FARM LAND */
                    const double FF_LAND = 4000.0;
                    const double FF_LAND_C = 4.02;
                    const int FF_LAND_D = 20;

                /* FARM ADJLAND */
                    const double FF_ADJLAND = 150.0;
                    const double FF_ADJLAND_C = 50.0;
                    const int FF_ADJLAND_D = 0;

                /* RUN FROM KNIGHT */
                    const double FR_KNIGHT = -8000.0;
                    const double FR_KNIGHT_C = 4.0;
                    const int FR_KNIGHT_D = 0;

                /* Disperse */
                    const double FD_FARMER = 0;
                    const double FD_FARMER_C = -1;
                    const int FD_FARMER_D = 1;


            /* KNIGHT */
                /* KILL FARMER */
                    const double KK_FARMER = 49.0;
                    const double KK_FARMER_C = 1.5;
                    const int KK_FARMER_D = 20;

                /* KILL KNIGHT */
                    const double KK_KNIGHT = 50.0;
    ...
    void FV_PROPAGATION(VVD& PRIORITY, double Value, double Change, int Distance, Pos P, string obstacle, int team, bool div, bool diagonal,  bool haunted){
        queue<S_BFS> Q;
        Q.push({ P, 0, Value });
        while (!Q.empty()) {
    ...
    void FV_WITCH_UPDATE(VVD& WP, VVD& KP, VVD& FP, const int& I, const int& J){
        ...
        //FV_PROPAGATION(FP, FR_FWITCH, FR_FWITCH_C, FR_FWITCH_D, { I, J }, "000", T_ALL, true, false, false);
    ...
    void FV_WITCH_MARK(VVB& HAUNTED, int id, int d){
        Unit W = unit(id);
        Pos P = W.pos;
        for (int i = 1 - d; i < d; ++i) {
            for (int j = 1 - d; j < d; ++j) {
                if (FB_Range(P, i, j) and FB_Dist(i, j, d)) {
    ...
    void FV_MOVE_W(int id, VVD& WP, VVD& KP, VVD& FP){
        Unit W = unit(id);
        Pos P = W.pos;
        Dir d = None;
    ...
    void FV_FARMER_UPDATE(VVD& WP, VVD& KP, VVD& FP, const int& I, const int& J, const Cell& c){
        ...
        if (u.player == 0) {
            FV_PROPAGATION(FP, FD_FARMER, FD_FARMER_C, FD_FARMER_D, { I, J }, "001", T_ALL, false, false, true);
        } else {
    ...
    void FV_MOVE_F(int id, VVD& WP, VVD& KP, VVD& FP){
        Unit F = unit(id);
        Pos P = F.pos;
        Dir d = None;
    ...
    void FV_LAND_UPDATE(VVD& WP, VVD& KP, VVD& FP, const int& I, const int& J, const Cell& c){
        if (c.owner == 0) {
            ...
            FP[I][J] += FF_ADJLAND;// FV_PROPAGATION(FP, FF_ADJLAND, FF_ADJLAND_C, FF_ADJLAND_D, { I, J }, "001", T_ENEMY, false, false, true);
            FV_PROPAGATION(FP, FF_LAND, FF_LAND_C, FF_LAND_D, { I, J }, "001", T_MINE, true, false, true);
        }
    ...
    virtual void play() {
        if (round() == 0) {
            ...
            W = witches(0);
            W1 =  witches(1); W2 = witches(2); W3 = witches(3);
            land = 0;
            FV_GET_LAND();
        }
        ...
        for (unsigned int i = 0; i < W.size(); ++i) {
            FV_WITCH_MARK(HAUNT, W[i],3);
            FV_WITCH_MARK(HAUNT, W2[i],4);
            FV_WITCH_MARK(HAUNT, W1[i],4);
            FV_WITCH_MARK(HAUNT, W3[i],4);
        }
